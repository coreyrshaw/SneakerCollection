...
public class SneakerController {
    ...
    public String addSneaker(HttpSession session ,String jordanModel,int jordanModelNumber, String jordanColor, int jordanReleaseYear, double jordanPrice,String jordanPhotoLink,boolean sneakerCopped) {
        User user = (User) session.getAttribute("user");
        Sneaker sneaker = new Sneaker(jordanModel,jordanModelNumber, jordanColor, jordanReleaseYear,jordanPrice,jordanPhotoLink,sneakerCopped,user);
    ...
    @RequestMapping(path = "/sneaker-copped", method = RequestMethod.GET)
    public String toggleSneaker(Model model, Integer sneakerID) {
        if (sneakerID != null) {
           Sneaker sneaker = sneakers.findOne(sneakerID);
          sneaker.copped = !sneaker.copped;
            sneakers.save(sneaker);
        }
        return "redirect:/";
    }
